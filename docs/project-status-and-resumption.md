...
- **Reviewed source baseline:**
  `a6664d6` (`Add read-only module trust review`).
- **Current behavior implementation:**
  `a6664d6` on `main`, including the redesigned TUI, Rust toolchain contract,
  ...
  independent portable-package verification, and the read-only local
  test-key-bound module package trust review.
- **CLI version:** `0.1.0`.
...
- **Current writes:** explicit user-local Unix/Windows-guarded store scaffolding
  ...
  with Windows runtime evidence still absent. Uninstall, cleanup, quarantine/restore,
  module lifecycle execution, and third-party execution remain unavailable.
...
the Windows process-host/probe slice at `39adb92`, and the module trust review
slice at `a6664d6`. Local `main` and
...
| `module-trust` | Test-key signature/staging contracts and local review adapter | Production roots/provenance/revocation |
...
Current validation for `a6664d6` on `aarch64-apple-darwin`:

- `cargo fmt --all -- --check` passed;
- `cargo test --workspace --locked` passed, including the new valid and
  identity-drift module-trust CLI cases;
