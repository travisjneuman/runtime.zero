...
- **Reviewed source baseline:**
  `0ce2cc6` (`Add exact quarantine restore lane`).
- **Current behavior implementation:**
 ...
 enumeration with bounded undeclared-file rejection.
  ...
  deletion, elevation, or module authority.
- **CLI version:** `0.1.0`.
...
the cache-root/restore slice at `0ce2cc6`.
...
was built from `7d0ed91`, before the `0ce2cc6` cache-root and restore changes;
the release package must be rebuilt before it can represent the current source.
...
- No uninstall, cleanup, broad restore, integrity remediation, or module
  ...
  receipt-bound cache/module lane described above.
