...
The seven-family catalog remains planned except for the bounded source-only
inventory lifecycle slice:
...
general install, activate, repair, migrate, upgrade, deactivate, or uninstall
modules, and the developer invocation does not provide production trust,
sandboxing, third-party execution, or lifecycle authority. The signed macOS
inventory v0 path is the bounded exception; public release distribution,
native sandboxing, and other modules remain blocked. Test-key signatures,
schemas, fixtures, process tests, and lifecycle plans remain non-production
evidence.
