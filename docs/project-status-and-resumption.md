...
| `modules install --developer-trial` | Local package read plus explicit runtime.zero-owned write | Staged module bytes and transaction/stage receipts; installed registry unchanged | Implemented test-key-only developer trial; no activation, invocation, production trust, or public distribution |
