...
- **Current behavior implementation:**
 ...
 enumeration with bounded undeclared-file rejection.
  ...
  as `installed_inactive` or `active`, reports missing or invalid evidence as
  `degraded`, and redacts unsafe registry identifiers. Status remains
  non-authorizing; only the signed macOS lifecycle path can publish `active`.
  ...
  is `degraded` rather than a false `installed_inactive` or `active` result.
  ...
  invocation authority. The signed macOS invocation lane now accepts only an
  active first-party.inventory package with complete immutable file evidence,
  binds its declared executable through the shared process host, and validates
  only a path-redacted read-only response. The developer-only invocation lane
  remains a separate promoted-fixture path; neither lane runs third-party code
  or claims a native sandbox.
  ...
  `lifecycle_state: "installed_inactive"` or `"active"`; schema 1 rejects any
  other explicit state, and status consumes the persisted value without
...
rz0 modules install --signed --dry-run <package> --signature <envelope.json> --trusted-test-key <key.json> --store-root <path> [--format text|json]
rz0 modules install --signed --apply <package> --signature <envelope.json> --trusted-test-key <key.json> --store-root <path> --challenge-issued-unix-seconds <seconds> --confirm <phrase> [--format text|json]
rz0 modules builtin <install|enable|disable|update|uninstall> --module-id <id> --dry-run|--apply [--store-root <path>] [--format text|json]
...
rz0 store init --dry-run|--yes [--store-root <path>] [--format text|json]
```
...
| module validation/install planning | No | No | Implemented planning plus bounded signed macOS first-party install path; optional complete-file-set review rejects undeclared files |
| modules builtin lifecycle | No | Runtime.zero private availability state | Seven compiled macOS capabilities support exact dry-run/apply install, enable, disable, and uninstall state changes; update is delivered by upgrading rz0; compiled bytes are never deleted |
...
| `modules install --signed` and lifecycle | Local signed macOS package plus explicit confirmation | Private module store, registry/install/lifecycle receipts, quarantine/recovery records | Implemented only for read-only first-party.inventory: enable/disable/update/invoke/uninstall/recover; no public distribution, native sandbox, or third-party trust |
| `modules trust verify` | No | No | Test-key-only exact package review; no production trust root or lifecycle authority |
| `modules status` | No | No | Path-redacted registry/receipt plus developer-staging view; reports staged, installed-inactive, active, or degraded |
...
The task-first TUI follows Home, Inventory, Evidence, Plan Review, Confirmation,
and Activity as a linear operator workflow. The loading shell precedes the
local snapshot, and `r` is the only explicit retry. Startup and refresh
workers carry a cancellation token; `q` cancels in-flight work, `r` cancels
the previous generation, and stale worker results cannot overwrite a newer
snapshot.
...
The seven-family package catalog remains optional distribution work, while all
seven capabilities are compiled into the macOS core and locally manageable.
Only the signed first-party.inventory package currently has a separately
installed executable lifecycle:

| Family | Current implementation | Major missing work |
| --- | --- | --- |
| Inventory/environment | Embedded read-only collector plus signed macOS first-party.inventory lifecycle v0 | Public release distribution, cross-platform lifecycle, native sandbox, and broader source/platform parity |
| Updater | Compiled-in macOS capability plus provider-driven plans and working macOS/Linux core executor | Windows isolation, rollback/recovery, manager/runtime matrix, release proof |
...
| Report/export | Compiled-in capability, strict module binary, and integrated foundation report | Signed package distribution and final-artifact platform proof |

The core can validate manifests/hashes and plan optional package installation.
Built-in capability availability is real local state controlled by the
foundation; compiled bytes are upgraded with rz0. The signed macOS inventory
v0 path provides the first separately installed lifecycle: install, enable,
invoke, disable, update, quarantine/uninstall, and recover. It uses a
caller-selected local release key, is not a public feed or native sandbox, and
does not grant third-party authority. Test-key signatures, schemas, fixtures,
process tests, and lifecycle plans outside these paths remain non-production
evidence.

The lifecycle must make those choices user-visible and reversible:
installed, disabled, enabled/active, degraded/blocked, and action-authorized
are distinct states. The v0 inventory path implements these states for its
bounded package. Disable stops module-owned collection, scheduling, network
work, UI actions, and mutation while preserving state; uninstall is a separate
explicit data-retention and rollback decision. Configuration, cross-platform
parity, and the full CLI/TUI parity surface remain future work.
