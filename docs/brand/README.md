# runtime.zero Brand System

The canonical public brand guide is [`../../BRAND.md`](../../BRAND.md).

This folder is reserved for deeper brand implementation notes as the project matures: color contrast matrices, logo usage, CLI/TUI output grammar, README/social assets, and website-specific design decisions.

Current standing direction:

- Canonical visual system: **Dossier Navy / Burnished Brass**.
- Deprecated phrase: **Dossier Navy / Oxide Copper**.
- Red is danger/error/destructive-only, never a default brand accent.
- Owner-provided candidate assets live under [`../../assets/brand/`](../../assets/brand/).

The website visual direction remains a separate approved lane. Its terminal mock now lags the real Rust-first Dossier Queue TUI, so future website work should use `BRAND.md`, the current TUI documentation, and candidate assets as source material. It must not mutate Cloudflare settings, add dependencies, or change release/bootstrap behavior without explicit approval.
