...
The module is not installed, loaded, or executed by `rz0`. The installed
foundation instead exposes `rz0 report`, which collects redacted live inventory
and private diagnostics in memory and calls the same support-contract builder.
The strict `support_report_input` envelope remains available to the separate
module. Development use is stdin/stdout only:
...
values, application/service names, process output, credentials, and free-form
...
The package remains incomplete for 1.0: it needs signed lifecycle artifacts, final-artifact runtime and accessibility
