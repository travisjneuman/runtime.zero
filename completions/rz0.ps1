...
Register-ArgumentCompleter -Native -CommandName rz0 -ScriptBlock {
    ...
    $commands = @('doctor','apps','cache','leftovers','recovery','restore','integrity','uninstall','modules','store','scan','monitor','toolchain','report','release','updates','completions','help','version')
    ...
    $candidates = if ($tokens.Count -le 2) {
        ...
        @('status','validate','install','lifecycle-plan','trust','--from','--store-root','--module-id','--from-state','--to-state','--from-version','--to-version','--transition-id','--developer-trial','--signature','--trusted-test-key','--challenge-issued-unix-seconds','--confirm','--dry-run','--apply','--format','--json','--help')
